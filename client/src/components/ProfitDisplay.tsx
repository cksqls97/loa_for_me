  ...
  return (
    <div className="mt-4 grid grid-cols-2 gap-4">
        ...
        <div className={`p-3 rounded-2xl border transition-all duration-300 ${profitStats.sellingProfit >= 0 ? 'bg-emerald-500/5 border-emerald-500/20 hover:bg-emerald-500/10' : 'bg-red-500/5 border-red-500/20 hover:bg-red-500/10'}`}>
            <h4 className={`text-center text-[10px] font-bold mb-2 uppercase tracking-widest ${profitStats.sellingProfit >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                판매 시 수익 (수수료 5%)
            </h4>
            ...
            <div className="flex flex-col items-center pt-1">
                <span className={`text-[9px] font-extrabold mb-0.5 uppercase tracking-[0.2em] ${profitStats.sellingProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                    순이익
                </span>
            ...
            {/* Hourly Profit */}
            <div className="mt-3 pt-2 border-t border-white/5 w-full flex justify-between items-center px-1">
        ...
        <div className={`p-3 rounded-2xl border transition-all duration-300 ${profitStats.usageProfit >= 0 ? 'bg-sky-500/5 border-sky-500/20 hover:bg-sky-500/10' : 'bg-red-500/5 border-red-500/20 hover:bg-red-500/10'}`}>
            <h4 className={`text-center text-[10px] font-bold mb-2 uppercase tracking-widest ${profitStats.usageProfit >= 0 ? 'text-sky-400' : 'text-red-400'}`}>
                본인 사용 이득
            </h4>
            ...
            <div className="flex flex-col items-center pt-1">
                <span className={`text-[9px] font-extrabold mb-0.5 uppercase tracking-[0.2em] ${profitStats.usageProfit >= 0 ? 'text-sky-600' : 'text-red-600'}`}>
                    절약
                </span>
